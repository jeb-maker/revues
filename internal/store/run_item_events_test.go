...
func TestUpdateRunItemStatusCreatesAuditEvent(t *testing.T) {
	...
	if err = st.UpdateRunItemStatus(ctx, run.ID, itemID, lead.ID, items.StatusOK, ""); err != nil {
	...
	if events[0].OldStatus.String != items.StatusPending || events[0].NewStatus != items.StatusOK {
	...
	if err = st.UpdateRunItemStatus(ctx, run.ID, itemID, lead.ID, items.StatusOK, "note"); err != nil {
	...
	if err = st.UpdateRunItemStatus(ctx, run.ID, itemID, lead.ID, items.StatusNOK, "problème"); err != nil {
	...
	if events[0].NewStatus != items.StatusNOK || events[0].Comment != "problème" {
