...
func TestUpdateItem_HTMXReturnsFragment(t *testing.T) {
	...
	if !strings.Contains(body, `id="complete-section"`) {
		t.Fatal("expected complete-section oob fragment")
	}
	if !strings.Contains(body, "Points obligatoires en attente") {
		t.Fatal("expected pending required blockers in complete section")
	}
	if !strings.Contains(body, "disabled") {
		t.Fatal("expected complete button disabled while required items pending")
	}
